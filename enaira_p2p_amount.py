...
if __name__ == "__main__":
    ...
    try:
        data = pd.read_csv('Training_data_mlflow_amt_p2p.csv')
    except Exception as e:
        logger.exception(
            "Unable to read csv. Error: %s", e)

    scaled_df = data[['month', 'day', 'hour', 'minute', 'merchant_label_', 'tier_level_','AMOUNT_Cleaned', 'current_state_', 'MailGroup_',
    ...
    with mlflow.start_run():
        ...
        if tracking_url_type_store != "file":
            ...
            mlflow.sklearn.log_model(rfc, "model", registered_model_name="rfc_amount_ptp")
        else:
